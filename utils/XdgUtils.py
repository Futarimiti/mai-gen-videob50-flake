...
def app_data_dir() -> Path:
    ...
    where $XDG_DATA_HOME would default to ~/.local/share if undefined.
    This is overridable by setting the environment variable $MAI_GEN_DATA_DIR.
    The directory will be created if had not been created.
    """
    base = os.environ.get(
        "MAI_GEN_DATA_DIR",
        default=os.path.join(os.environ.get(
            "XDG_DATA_HOME", os.path.expanduser("~/.local/share")), "mai-gen-videob50"))
